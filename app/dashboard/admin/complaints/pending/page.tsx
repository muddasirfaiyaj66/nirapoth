...
export default function PendingComplaintsPage() {
  ...
  const updateComplaintMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["admin-complaints-pending"] });
    },
    onError: (error) => {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Review and process pending citizen complaints
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Badge variant="destructive" className="text-sm">
            <Clock className="h-3 w-3 mr-1" />
            {totalComplaints} Pending
          </Badge>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {complaints.filter((c) => c.type === "TRAFFIC").length}
            </div>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {complaints.filter((c) => c.type === "INFRASTRUCTURE").length}
            </div>
          </CardContent>
        </Card>
      </div>
